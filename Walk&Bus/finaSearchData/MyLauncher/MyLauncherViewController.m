...
@synthesize  busService=_busService;
NSString *temps;
...
-(void)loadView {
	[super loadView];
	_busService=[[ViewController alloc]init];
...
-(void)launcherViewItemSelected:(MyLauncherItem*)item {
   ...
   // NSLog(@"%@",temps);
    ...
    ViewController *bus=[[ViewController alloc]init];
    bus.busServiceName=temps;
                ...
                if ((version = [item objectForKey:@"myLauncherViewItemVersion"])) {
                    if ([version intValue] == 2) {
                        [savedPage addObject:[[MyLauncherItem alloc]
                                              ...
                                              busName:[item objectForKey:@"busName"]]];
                    }
                } else {
                    [savedPage addObject:[[MyLauncherItem alloc]
                                           initWithTitle:[item objectForKey:@"title"]
